...
- **五个同级大模块**：旅行信息 / 每日行程 / 行前准备 / 餐饮 / 记账，顶栏直接切换（`data-site-view` + hash 路由）。
  每日行程、行前准备都可深链访问（`#itinerary` / `#prep`）。
- **每日行程**默认全部展开（点天头收起），顶部有「总览 + DAY01-DAY11」单天查看按键，
  新增行程的表单（日期/时间/类型/内容）在查看按键上方，提交后自动切到对应天。
- **行程提醒**是三卡轮播（上一件·已完成 / 最近一件 / 下一件），默认展示中间一张，左右滑动切换；
  下方「未完成 / 完成」按钮作用于当前可见卡，点「完成」会把该条目的时间更新为完成时刻并写回每日行程。
- **旅行信息**新增「住宿安排」（入住/退房/晚数卡片轮播）与「门票预订」（复用票券详情弹窗），
  交互与航班行程一致。
...
- **行前准备**每个分类头部有「批量责任人」下拉，一键划走整组；每项仍保留单独的责任人下拉。标为「共同」的项两人各带一份：
